    ...
    def daily_return(df):
        try:
            ...
            for x in range(len(df["Close"])):  # for loop to calculate daily return
                if x == 0:
                    df[["Daily_Return"][x]] = (
                        0  # setting first daily return as 0 for easy calculation removing the first row later
                    )
                else:
                    df["Daily_Return"][x] = (
                        (df["Close"][x] - df["Close"][x - 1]) / df["Close"][x - 1] * 100
                    ...
                    df["Daily_Return"] = df["Daily_Return"].round(
                        2
