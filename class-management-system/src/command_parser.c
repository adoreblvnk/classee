...
void processCommand(StudentRecord **root, char *input, const char *db_filename) {
  ...
  if (util_strcasecmp(command, "OPEN") == 0) {
    ...
    if (args) {
      ...
      log_transaction("insert", id, nameBuffer, programmeBuffer, mark);
