...
def analyze_up_down_runs(df, start_date=None, end_date=None):
    ...
    median_down = (
        float(pd.Series([r[1] for r in down_runs]).median()) if down_runs else 0
    )

    return {
        "longest_up": {
            "length": longest_up[1] if longest_up else 0,
            "start_date": df["Date"].iloc[longest_up[2]].strftime("%Y-%m-%d")
            if longest_up and len(longest_up) >= 4
            else None,
            "end_date": df["Date"].iloc[longest_up[3]].strftime("%Y-%m-%d")
            if longest_up and len(longest_up) >= 4
            else None,
        },
        "longest_down": {
            "length": longest_down[1] if longest_down else 0,
            "start_date": df["Date"].iloc[longest_down[2]].strftime("%Y-%m-%d")
            if longest_down and len(longest_down) >= 4
            else None,
            "end_date": df["Date"].iloc[longest_down[3]].strftime("%Y-%m-%d")
            if longest_down and len(longest_down) >= 4
            else None,
        },
